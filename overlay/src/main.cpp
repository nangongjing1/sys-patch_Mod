...
public:
    ...
    tsl::elm::Element* createUI() override {
        ...
        list->addItem(config_noacidsigchk1.create_list_item("禁用ACID签名校验_1.0.0-9.2.0"));
        list->addItem(config_noacidsigchk2.create_list_item("禁用ACID签名校验_1.0.0-9.2.0"));
        list->addItem(config_noncasigchk1.create_list_item("禁用NCA签名校验_1.0.0-3.0.2"));
        list->addItem(config_noncasigchk2.create_list_item("禁用NCA签名校验_4.0.0-16.1.0"));
        list->addItem(config_noncasigchk3.create_list_item("禁用NCA签名校验_17.0.0+"));
        list->addItem(config_nocntchk1.create_list_item("禁用CNT签名校验_1.0.0-18.1.0"));
        list->addItem(config_nocntchk2.create_list_item("禁用CNT签名校验_19.0.0+"));

        list->addItem(new tsl::elm::CategoryHeader("Loader - 0100000000000001"));
        list->addItem(config_noacidsigchk4.create_list_item("禁用ACID签名校验_10.0.0+"));

        list->addItem(new tsl::elm::CategoryHeader("ERPT - 010000000000002B"));
        list->addItem(config_no_erpt.create_list_item("禁用ERPT"));
        ...
        list->addItem(config_olsc3.create_list_item("模拟OLSC_19.0.0+"));
        ...
        if (does_file_exist(LOG_PATH)) {
            ...
            ini_browse([](const mTCHAR *Section, const mTCHAR *Key, const mTCHAR *Value, void *UserData){
                ...
                if (status.starts_with("修补")) {
                    ...
                    const auto display_value = detail.empty() ? std::string{"修补"} : "已修补";
                    // const auto display_value = detail.empty() ? std::string{"修补"} : "已修补 @ " + detail;
